...
export default async function AdminPage() {
  ...
  const rows = (profiles ?? []).flatMap((p) => {
    ...
    return userProgress.map((up) => ({
      ...
      pre: surveyTotal(surveyByUserModule.get(`${p.id}|${up.module_slug}|pre`)),
      post: surveyTotal(
        ...
        <div className="site-container py-12">
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2">
            {impact.map(({ mod, pre, post, completions, started }) => (
              <div
                key={mod.slug}
                className="rounded-2xl border border-lav-deep bg-white p-6 shadow-[0_2px_10px_rgba(70,45,115,0.08)]"
              >
          ...
          <div className="overflow-x-auto rounded-2xl border border-lav-deep bg-white shadow-[0_2px_10px_rgba(70,45,115,0.08)]">
